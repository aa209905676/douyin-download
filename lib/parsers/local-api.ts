...
export class LocalApiParser extends BaseParser {
  name = 'LocalAPI'
  priority = 2
  ...
  constructor(config?: { baseUrl?: string; timeout?: number }) {
    super()
    this.baseUrl = config?.baseUrl || 'http://127.0.0.1:6666'
  ...
  async getInfo(url: string): Promise<VideoInfo> {
    ...
    try {
      ...
      if (result.code !== 200) {
        throw new ParserError(
          `Local API returned error: ${result.code} - ${result.message || 'Unknown error'}`,
          'API_ERROR',
          this.name
        )
      }
      ...
      throw new ParserError(
        `Failed to get video info from local API: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'NETWORK_ERROR',
        this.name
      )
    }
  ...
  private mapToVideoInfo(data: Record<string, unknown>): VideoInfo {
    // 根据API返回的数据结构进行映射（与HybridAPI相同的格式）
